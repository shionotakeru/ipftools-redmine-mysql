 ...
 * ソース規模情報を収集する処理を行う。
 */
public class SourceScaleCountTracGit extends SourceScaleCountGit {

    /**
     * ソース規模データ収集(Trac - Git用) 
     * 
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (projectId == null) {
                throw new Exception(
                        resource.getString("trac.git.error.parameter.length.check"));
            }
            ...
            if (oldRev != null && newRev == null) {
                // 旧リビジョンが指定されていて、新リビジョンが指定されていない場合
                // 旧リビジョンには新リビジョンのがセットされているので、
                // source_scale テーブルの最新リビジョンを取得して旧リビジョンにセットする
                // source_scale テーブルが空の場合、旧リビジョンに null をセットし、
                // 全リビジョンを収集対象とする。
                newRev = oldRev;
                oldRev = getLatestRevision(configInfo, projectId);
            }
            
            // リポジトリパス取得
            String repoPath = getTracRepoPath(projectId);
            if (StringUtils.isBlank(repoPath)) {
                // リポジトリパスが設定されていない
                throw new Exception(
                        resource.getString("trac.error.repopath.no.setting"));
            }
            ...
            if (! (repoPathFile.isDirectory() && repoPathFile.canRead())) {
                ...
                throw new Exception(String.format(
                        resource.getString("trac.error.repopath.cannot.read"),
                        repoPath));
            }
